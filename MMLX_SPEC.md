...
* Streaming songs: `pub fn <name>() -> NoteIterator` yields the intro once, then the loop body forever (`gen!`/`yield_!` from our vendored genawaiter, one body per pull, boxed at the song boundary). Voices and music read top-to-bottom in one flow inside the generator; `bar!` keeps each bar its own codegen unit so song-only rebuilds stay in seconds. The server pages one body per loop wrap with bounded memory instead of replaying one collected stream; highlight ordinals restart per body, and pause/stop/resume/reload keep their finite-song semantics (reload and reset restart the performance from the top).
