...
* `mmlx-server`: stdin commands `src <tmpfile> <index> [section] | sfxsrc ... | play | stop | reset | loop on|off | preview <ch> <tempo> <token> | audition <midi 0-127> <ym|psg>` (audition plays one pitch now on a private preview voice, no JIT); stdout `pos <tick> <t0> <t1> <t2> <t3> | ended | err ...`. Compiles `mmlx-songs` cdylib/JIT or interprets event stream directly; patches playback in place preserving position (lotw `editor/extension.js` protocol, adapted from `song/section/line` to `ser/par` + section index). Generator bodies resolve once per identical tree (bounded cache), so replays and loop wraps skip re-resolution.
* VSCode extension (full lotw port): CodeLens `▶/⏸ ⏹ 🔁` per song fn + per top-level `ser!` section; yellow highlight of sounding atoms per channel (map server token index → source span via tree-sitter; `env!` = 1 token); debounced live reload; cursor audition (note under cursor on arrival, edited note confirmed; native preview voices) with identical-`load` memoing so repeat plays skip evcxr; follow pages with minimal scroll inside a dead-zone instead of re-centering every note.
* Streaming songs: `pub fn <name>() -> NoteIterator` yields the intro once, then the loop body forever (`gen!`/`yield_!` from our vendored genawaiter, one body per pull; our fork's `gen!` boxes the stream itself, so songs end with a bare `gen!({...})`). Voices and music read top-to-bottom in one flow inside the generator; `bar!` keeps each bar its own codegen unit so song-only rebuilds stay in seconds. The server pages one body per loop wrap with bounded memory instead of replaying one collected stream; highlight ordinals restart per body, and pause/stop/resume/reload keep their finite-song semantics (reload and reset restart the performance from the top).
