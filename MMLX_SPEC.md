...
* Streaming songs: `pub fn <name>() -> NoteIterator` yields the intro once, then the loop body forever (`gen!`/`yield_!`, one body per pull, boxed at the song boundary). Voices, intro and loop bodies are plain `let`s outside the generator (hoverable — rust-analyzer cannot see through the `gen!` transform, so only the 4-line handoff lives inside it: intro moved once, loop via `Arc` clone per cycle). The server pages one body per loop wrap with bounded memory instead of replaying one collected stream; highlight ordinals restart per body, and pause/stop/resume/reload keep their finite-song semantics (reload and reset restart the performance from the top).
